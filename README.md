...
1. 你在学习过程中补充到 `.mlx` 里的公开笔记、理解、改写和纠错，都视为主线讲义的一部分。
2. 后续我继续完善内容时，直接在你修改后的主线 `.mlx` 基础上补充，不再为了“加厚”另建新文件。
3. 每次推送 GitHub 前，我只做隐私和敏感信息检查，例如本机真实数据路径、token、API key、大型私有数据文件。
4. 已存在的 `_加厚版.mlx` 保留为历史补充训练包，后续新增内容优先合入主线讲义。
